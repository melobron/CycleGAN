...
class TrainCycleGAN:
    def __init__(self, args):
        ...
        # Loss
        self.criterion_GAN = torch.nn.MSELoss(reduction='mean')
        self.criterion_cycle = torch.nn.L1Loss(reduction='mean')
        self.criterion_identity = torch.nn.L1Loss(reduction='mean')
        ...
        # Dataset
        self.dataset = Paired(domain1=args.domain1, domain2=args.domain2, train=True, transform=transform)
        self.train_dataset, _ = train_test_split(self.dataset, train_size=args.train_size, shuffle=False)
        ...
        # Save Paths
        self.project_name = '{}2{}'.format(args.domain1, args.domain2)
        self.checkpoint_path = './checkpoints/{}_with_buffer/'.format(self.project_name)
        if not os.path.exists(self.checkpoint_path):
            os.makedirs(self.checkpoint_path)

        self.result_path = './results/{}_with_buffer/'.format(self.project_name)
    ...
    def train(self):
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            # Training
            for batch, (real_A, real_B) in enumerate(self.train_dataloader):
                ...
                print('[Epoch {}][{}/{}] | loss: G_total={:.3f} G_identity={:.3f} G_GAN={:.3f} G_Cycle={:.3f} D_total={:.3f}'.format(
                    epoch, (batch + 1) * self.batch_size, len(self.train_dataset), loss_G.item(),
            ...
            if epoch % 50 == 0 or epoch == self.n_epochs:
                torch.save(self.netG_A2B.state_dict(), os.path.join(self.checkpoint_path,
                                                                    'netG_A2B_train_size_{}_{}epochs.pth'.format(self.args.train_size, epoch)))
                torch.save(self.netG_B2A.state_dict(), os.path.join(self.checkpoint_path,
                                                                    'netG_B2A_train_size_{}_{}epochs.pth'.format(self.args.train_size, epoch)))
                torch.save(self.netD_A.state_dict(), os.path.join(self.checkpoint_path,
                                                                    'netD_A_train_size_{}_{}epochs.pth'.format(self.args.train_size, epoch)))
                torch.save(self.netD_B.state_dict(), os.path.join(self.checkpoint_path,
                                                                    'netD_B_train_size_{}_{}epochs.pth'.format(self.args.train_size, epoch)))
